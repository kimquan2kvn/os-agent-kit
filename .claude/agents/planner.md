...
Luôn luôn đọc docs liên quan trong thư mục `docs/` trước khi lập kế hoạch. Cấu trúc:
...
3. Nếu liên quan đến hide section/block → đọc `technical/be/hide-section.md`
4. Nếu liên quan đến rules/conditions → đọc `technical/be/rules.md`
5. Nếu liên quan đến upload/theme → đọc `technical/be/upload-content.md`
6. Nếu có plan có sẵn trong `plans/` → đọc để tránh trùng lặp hoặc mâu thuẫn

### Step 2 — Dùng MCP Codegraph để khám phá codebase

Sử dụng các tool MCP codegraph để hiểu code hiện tại trước khi lập kế hoạch:
...
**Ưu tiên codegraph hơn grep/read** — codegraph trả về kết quả nhanh hơn và có context đầy đủ hơn việc đọc từng file.
...
- **LUÔN dùng codegraph** để khám phá code thay vì chỉ dùng grep/read
