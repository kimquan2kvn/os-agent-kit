...
1. **Project Instructions**: dùng `read_file(".claude/CLAUDE.md")`
2. **Coding Convention**: dùng `read_file(".claude/skills/coding-convention.md")`
3. **Project Docs liên quan**: dùng `read_file` trên các file trong `docs/` liên quan đến feature đang review
...
| Lưu findings | `save_memory` |

## Quy trình Review

### Bước 1: Thu thập context
1. `read_file(".claude/CLAUDE.md")` và `read_file(".claude/skills/coding-convention.md")`
...
4. `read_file` các docs liên quan trong `docs/` nếu feature có doc riêng
...
Dùng `save_memory` để tích lũy kiến thức về codebase sau mỗi review:

```
save_memory(key="reviewer/patterns", value="<pattern tốt/xấu tìm thấy>")
save_memory(key="reviewer/conventions", value="<convention đặc thù của team>")
save_memory(key="reviewer/risky-areas", value="<vùng code nhạy cảm hay gây bug>")
```
