{%- comment %}BSS Login{% endcomment -%}{%- assign bss_locale =  request.locale.iso_code -%} {%- assign bss_translation = shop.metaobjects['app--2811029--bss_lock__metaobject'].translation[bss_locale].value -%}{% assign translation_rule = origin_rule_id | append : "" %}{%- comment %}BSS-Login{% endcomment -%}
<style>
@keyframes spin {
...
{%- if form_style_type == '0' -%}
  <div class="page-width bsscommerce-passcode-container bss-lock-passcode-container-{{ origin_rule_id }} bsscommerce-passcode-form-style-{{ form_style_type }}">
    <div class="bss-commerce-passcode-container-1" >
      {% if passcode_image_url != blank %}
      <div class="passcode-image-container">
        <img src="{{ passcode_image_url }}" alt="" style="max-width:100%;max-height:200px;object-fit:contain;display:block;margin:0 auto;" onerror="this.style.display='none'" />
      </div>
      {% endif %}
      ...
      <div class="bss-commerce-passcode-container-2" >
        <div id="BsscommercePasswordForm">
          <div class="bss-commerce-password-form-vertical">
            <span class="bss-commerce-passcode-input-label">{{ passcode_input_label }}</span>
            <form id="bsscommerce-ltap-password" novalidate >
            ...
            {% if is_pc_request == "true" %}
              <div class="bss-passcode-request bss-btn">{{ bss_translation[translation_rule].request_passcode_button | default: "Request passcode" }}</div>
            {% endif %}
            ...
            {% for lock_id in bss_lock_ids %}
              {% unless bss_opened_lock_ids contains lock_id %}
				...
				{% if lock_id == '10741' %}
				{% if passAttrCheck-15533 and passAttrCheck-16496 %}
					{% assign passAttrCheck-15533 = "*bss*" | append: passAttrCheck-15533 | append: "*bss*" %}{% assign passAttrCheck-16496 = "*bss*" | append: passAttrCheck-16496 | append: "*bss*" %}
						{% if bss-lock-pc-escape-15533 contains passAttrCheck-15533 or bss-lock-pc-escape-16496 contains passAttrCheck-16496 %}
						{% else %}
							
        <div class="form-message form-message--error" style="display: flex; justify-content: flex-start; margin-top: 4px; color: red;">
    ...
    <div class="modal-dialog">
        <div class="modal-dialog__modal">
            <div class="modal_heading">
                <h2 class="modal_heading__text">{{ bss_translation[translation_rule].request_passcode_modal_title | default: "Request passcode" }}</h2>
                    ...
                    <div class="modal__stack">
                        <div class="modal__stack-item">
                            <h3 class="modal__field-name">
                                {{ bss_translation[translation_rule].request_passcode_email_label | default: "Email" }} 
                            ...
                            <p class="modal__field-desc">{{ bss_translation[translation_rule].request_passcode_email_help_text | default: "Passcode will be sent to this email" }}</p>
                        </div>
                        <div class="modal__stack-item">
                            <h3 class="modal__field-name">
                              {{ bss_translation[translation_rule].request_passcode_name_label | default: "Name" }}
                            </h3>
                            <input 
                        ...
                        <div class="modal__stack-item">
                            <h3 class="modal__field-name">{{ bss_translation[translation_rule].request_passcode_message_label | default: "Message" }}</h3>
                            <textarea
            ...
            <div class="modal__footer">
                <div class="modal__footer-action">
                    <div class="modal__button modal__btn-cancel">{{ bss_translation[translation_rule].request_passcode_cancel_action | default: "Cancel" }}</div>
                    <button class="modal__button modal__button--primary" type="submit">{{ bss_translation[translation_rule].request_passcode_submit_action | default: "Send request" }}</button>
                </div>
            </div>
        </div>
    </div>
      ...
      <div class="modal-dialog__modal">
          <div class="modal__body">
              ...
              >
                  ...
                  <h4 style="text-align: center;">{{ bss_translation[translation_rule].request_passcode_success_message | default: "Request sent, please wait for approval" }}</h4>
                  <p style="text-align: center;" class="bss-passcode-sent-to">{{ bss_translation[translation_rule].request_passcode_send_to | default: "Passcode will be sent to" }} {% if customer %}{{ customer.email }}{% endif %}</p>
                  <button class="modal__button bss-btn ok">{{ bss_translation[translation_rule].request_passcode_ok_btn | default: "OK" }}</button>
              </section>
          </div>
      </div>
  </div>
        ...
        <div class="modal-dialog__modal">
            <div class="modal__body">
                ...
                >
                    ...
                    <h4 style="text-align:center;">{{ bss_translation[translation_rule].request_passcode_fail_message | default: "You are only allowed to send 1 request in 1 minutes!" }}</h4>
                    <button class="modal__button bss-btn ok">{{ bss_translation[translation_rule].request_passcode_ok_btn | default: "OK" }}</button>
                </section>
            </div>
        </div>
...
{%- capture authentication -%}{"domain":"{{ shop.permanent_domain }}","ruleId":{{ origin_rule_id }}}{%- endcapture -%}
...
<script type="text/javascript">
  ...
  formPasscodeReq.addEventListener("submit", async function (e) {
      e.preventDefault();
      if (!email.value) {
        emailError.textContent = "{{ bss_translation[translation_rule].request_passcode_email_required | default: "Email is required" }}";
        emailError.style.display = 'block';
        return;
      }
      if (!isValidEmail(email.value)) {
        emailError.textContent = "{{ bss_translation[translation_rule].request_passcode_email_invalid | default: "Invalid email format" }}";
        emailError.style.display = 'block';
        return;
      }
      ...
      try {
        ...
        if(res.ok) {
          ...
          {% unless customer %}
            ...
            passcodeSendTo.innerHTML = `{{ bss_translation[translation_rule].request_passcode_send_to | default: "Passcode will be sent to" }} ${email.value}`
          {% endunless %}
        }
        modal.style.display = "none";
        emailError.style.display = "none";
  ...
  let formData = new FormData();
				...
				{% if rule_id == '10741' %}
					{% assign all_passcode_condition_id = '15533,16496' | split: ',' %}
					{% assign list_passcode_case_sensitive_id = '15533,16496' | split: ',' %}
					{% for condition_id in all_passcode_condition_id %}
						{% assign current_index = passcode_condition_ids | find_index: condition_id %}
						{% assign escape_attr_key = 'bss-lock-pc-escape-' | append: condition_id %}
					{% unless cart.attributes[escape_attr_key] contains cart.attributes[cart_attributes_success_names[current_index]] %}
							{% if all_passcode_condition_id contains condition_id and list_passcode_case_sensitive_id contains condition_id %}
					...
					{% for condition_id in all_passcode_condition_id %}
                        {% assign current_index = passcode_condition_ids | find_index: condition_id %}
						{% assign escape_attr_key = 'bss-lock-pc-escape-' | append: condition_id %}
						{% unless cart.attributes[escape_attr_key] contains cart_attributes_check_names[current_index] %}
							{% if all_passcode_condition_id contains condition_id and list_passcode_case_sensitive_id contains condition_id %}
  ...
  document.getElementById('bsscommerce-ltap-password').addEventListener('submit', function(e) {
    ...
    {% if is_call_api == "true" %}
      ...
      if(customerId) {
        ...
        let data = JSON.stringify({
          ...
          xmlHttp.setRequestHeader("Authentication", "Bearer {{ authentication }}");
          xmlHttp.send(data);
        }
    {% endif %}
    ...
    function getSessionId() {
      function generateId() {
        ...
        window.localStorage.setItem('bss-login-session-id', randomID);
        return randomID;
      }
      let lsSession = window.localStorage.getItem('bss-login-session-id');
      let cookieSession = getCookie('_shopify_y');
      return lsSession || cookieSession || generateId();
    }
...
{% if rule_id == '10741' %}
      ...
      if (window?.Shopify?.shop === 'cg-habitats.myshopify.com') {
